...
struct PlanStuffIntent: AppIntent, IntentPerformer {
    ...
    static func perform(_ input: Input) async throws -> Output {
        ...
        // Derive lightweight preferences from past data
        let all = (try? context.fetch(FetchDescriptor<Stuff>())) ?? []
        let liked = all.filter { $0.isCompleted || ($0.lastFeedback ?? 0) > 0 || $0.score >= 80 }
        let disliked = all.filter { ($0.lastFeedback ?? 0) < 0 || $0.score <= 20 }
        ...
        // Few-shot example to enforce structure
        let example = """
            Example PlanItem:
